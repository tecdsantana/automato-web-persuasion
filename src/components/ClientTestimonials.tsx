...
const ClientTestimonials = () => {
  ...
  const technologies = ["N8N", "AWS", "Supabase", "Make", "Python", "WhatsApp", "Google", "JavaScript"];
  return <section id="depoimentos" className="section-padding bg-automato-dark-blue">
      <div className="container mx-auto container-padding">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {testimonials.map(testimonial => <div key={testimonial.id} className="bg-automato-dark-blue/50 p-6 md:p-8 rounded-xl shadow-md hover:shadow-xl transition-shadow border border-automato-gold/20 futuristic-card">
              <div className="flex items-center mb-4">
                {[...Array(testimonial.stars)].map((_, i) => <Star key={i} className="h-5 w-5 fill-automato-gold text-automato-gold" />)}
              </div>
              <p className="text-white/80 mb-6 italic">"{testimonial.text}"</p>
              <div className="flex items-center">
                <img src={testimonial.avatar} alt={testimonial.name} className="w-12 h-12 rounded-full mr-4 object-cover border border-automato-gold/30" />
                <div>
